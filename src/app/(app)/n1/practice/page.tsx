...
import kanjiData from "@/data/n1/kanji.json";
import vocabData from "@/data/n1/vocabulary.json";
...
function generateQuestions(quizType: QuizType): QuizQuestion[] {
  switch (quizType) {
    case "kanji-meaning": return generateKanjiQuestions(kanjiData, "n1", "kanji-meaning", 10);
    case "kanji-reading": return generateKanjiQuestions(kanjiData, "n1", "kanji-reading", 10);
    case "meaning-kanji": return generateKanjiQuestions(kanjiData, "n1", "meaning-kanji", 10);
    case "vocab-meaning": return generateVocabQuestions(vocabData, "n1", "word-meaning", 10);
    case "meaning-vocab": return generateVocabQuestions(vocabData, "n1", "meaning-word", 10);
    case "vocab-reading": return generateVocabQuestions(vocabData, "n1", "word-reading", 10);
    case "grammar-meaning": return generateGrammarQuestions(grammarData, "n1", "pattern-meaning", 10);
    case "meaning-grammar": return generateGrammarQuestions(grammarData, "n1", "meaning-pattern", 10);
    default: return [];
  }
}

export default function N1PracticePage() {
  const [quizType, setQuizType] = useState<QuizType>(null);
  const [questions, setQuestions] = useState<QuizQuestion[]>([]);
  const [quizKey, setQuizKey] = useState(0);

  const startQuiz = useCallback((qt: QuizType) => {
    setQuizType(qt);
    setQuestions(generateQuestions(qt));
    setQuizKey((k) => k + 1);
  }, []);
  ...
  if (quizType && questions.length > 0) {
    ...
    return (
      <div className="max-w-lg mx-auto space-y-4">
        ...
        <QuizEngine key={quizKey} questions={questions} title={`Quiz N1 · ${qt?.title ?? ""}`} onComplete={handleComplete} />
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {QUIZ_TYPES.map((qt) => (
          <Card key={qt.id} className="group cursor-pointer border-border bg-card hover:border-violet-500/30 transition-all duration-200 hover:scale-[1.02]" onClick={() => startQuiz(qt.id)}>
