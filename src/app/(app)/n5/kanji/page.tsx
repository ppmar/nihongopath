...
export default function N5KanjiPage() {
  ...
  const questions = useMemo(
    () => generateKanjiQuestions(kanjiData, "n5", quizMode, 10),
    [quizMode, quizKey]
  );
  ...
  if (mode === "quiz") {
    return (
      <div className="max-w-lg mx-auto space-y-4">
        ...
        <QuizEngine key={quizKey} questions={questions} title="Quiz Kanji N5" />
      </div>
    );
  }
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {kanjiData.length} kanji essentiels · Clique pour retourner
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <Button
            ...
            className="gap-2 bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 shrink-0"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {filtered.map((kanji) => (
          <KanjiCard key={kanji.kanji} data={kanji} />
        ))}
      </div>

      {filtered.length === 0 && (
        <p className="text-center text-muted-foreground py-12">
          Aucun kanji trouvé pour &quot;{search}&quot;
        </p>
      )}
    </div>
  );
