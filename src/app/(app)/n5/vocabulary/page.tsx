...
export default function N5VocabularyPage() {
  ...
  const startQuiz = useCallback((qm?: QuizMode) => {
    const m = qm ?? quizMode;
    setQuestions(generateVocabQuestions(vocabData, "n5", m, 10));
    setQuizKey((k) => k + 1);
    setMode("quiz");
  }, [quizMode]);
  ...
  const changeMode = useCallback((m: string) => {
    ...
    setQuizMode(qm);
    setQuestions(generateVocabQuestions(vocabData, "n5", qm, 10));
    setQuizKey((k) => k + 1);
  }, []);
  ...
  if (mode === "quiz" && questions.length > 0) {
    return (
      <div className="max-w-lg mx-auto space-y-4">
        ...
        <QuizEngine key={quizKey} questions={questions} title="Quiz Vocabulaire N5" />
      </div>
    );
  }
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">{vocabData.length} mots essentiels pour le JLPT N5</p>
        </div>
