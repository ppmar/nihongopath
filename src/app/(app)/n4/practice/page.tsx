...
import kanjiData from "@/data/n4/kanji.json";
import vocabData from "@/data/n4/vocabulary.json";
import grammarData from "@/data/n4/grammar.json";
...
function generateQuestions(quizType: QuizType): QuizQuestion[] {
  switch (quizType) {
    case "kanji-meaning": return generateKanjiQuestions(kanjiData, "n4", "kanji-meaning", 10);
    case "kanji-reading": return generateKanjiQuestions(kanjiData, "n4", "kanji-reading", 10);
    case "meaning-kanji": return generateKanjiQuestions(kanjiData, "n4", "meaning-kanji", 10);
    case "vocab-meaning": return generateVocabQuestions(vocabData, "n4", "word-meaning", 10);
    case "meaning-vocab": return generateVocabQuestions(vocabData, "n4", "meaning-word", 10);
    case "vocab-reading": return generateVocabQuestions(vocabData, "n4", "word-reading", 10);
    case "grammar-meaning": return generateGrammarQuestions(grammarData, "n4", "pattern-meaning", 10);
    case "meaning-grammar": return generateGrammarQuestions(grammarData, "n4", "meaning-pattern", 10);
    default: return [];
  }
}

export default function N4PracticePage() {
  ...
  if (quizType && questions.length > 0) {
    ...
    return (
      <div className="max-w-lg mx-auto space-y-4">
        ...
        <QuizEngine key={quizKey} questions={questions} title={`Quiz N4 · ${qt?.title ?? ""}`} onComplete={handleComplete} />
      </div>
    );
  }
